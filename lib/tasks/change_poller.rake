...
namespace :petitions do
  ...
  task populate_petitions: :environment do
    ...
    Petition.pluck(:petition_id).each do |petition_id|
      ...
      ChangeDataService.new.comments_for_petition(petition_id: Petition.where(source: 'whitehouse.gov').first.petition_id, client: WhitehouseClient.new, comment_builder: WhitehouseCommentBuilder, offset_amount: 1000)
    end
  end
end
